...
import { asset, money, rentalPeriod, catalogProducts, type Product, type ProductVariant } from '@/data/catalog';
...
export function HomePackage({ product, variant, choose, add, selected }: { product: Product; variant: ProductVariant; choose: (id: string) => void; add: () => void; selected: boolean }) {
  return <section className="home-package page-width section-space" id="budget" data-sc-act="flow">
    ...
    <div className="package-example">
      ...
      <div className="package-copy"><p className="eyebrow">ГОТОВОЕ РЕШЕНИЕ ДЛЯ СТОЛА</p><h3>{product.name}</h3><p className="package-intro">Мебель и текстиль уже сочетаются. Вы решаете, сколько деталей добавить в свою свадьбу.</p>
        <VariantPicker product={product} variant={variant} choose={choose} surface="page" />
        <div className="package-price" aria-live="polite"><strong>{money(variant.price)}</strong><span>{variant.unit}<br />{rentalPeriod(variant.rentalDays)}</span></div>
        ...
        <p className="package-note">Цена за выбранный состав. Доставка и монтаж отдельно. Наличие и итоговую стоимость подтверждает команда.</p>
      </div>
    </div>
  </section>;
}
...
export function HomeProof() {
  const quietGarden = catalogProducts.find(product => product.category === 'snake-tables' && product.name.includes('Тихий сад'));
  return <section className="home-proof" id="expectation" data-sc-act="flow"><div className="page-width section-space">
    ...
    <div className="proof-bottom"><p>Пример из портфолио Амели с деталями «Тихого сада». Флористика, свечи и оформление площадки на фото дополняют готовый комплект и рассчитываются отдельно.</p><a className="text-link" href={quietGarden ? productHref(quietGarden.id) : categoryHref('snake-tables')}>Рассмотреть «Тихий сад» <ArrowUpRight size={19} /></a></div>
